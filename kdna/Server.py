...
class Server:
    ...
    def add(self):
        ...
        if index_servers is not None:
            ...
            existing_servers = self.extract_existing_servers(lines, index_servers, index_auto_backups)
            if str(self.id_server) in existing_servers:
                print(f"Erreur : L'id du serveur \"{self.id_server}\" existe déjà dans la section [servers].")
                return

            # On vérifie de l'unicité de l'alias
            if self.alias is not None:
                existing_aliases = self.extract_existing_aliases(lines, index_servers, index_auto_backups)
                if self.alias in existing_aliases:
                    print(f"Erreur : L'alias \"{self.alias}\" existe déjà dans la section [servers].")
                    return
    ...
    def delete(id_or_alias, by_alias=False):
        ...
        if line_to_delete is not None:
            ...
            if len(line_elements) >= 4 and deleted_line.strip():
                ...
                print(f"L'élément avec l'{element_type} {id_or_alias} a été supprimé de la section [servers].")
            # Sinon on affiche un message d'erreur
            else:
                print(f"Erreur : Aucun élément trouvé avec l'{element_type} \"{id_or_alias}\" dans la section [servers].")
        # Sinon on affiche un message d'erreur
        else:
    ...
    def find_line_to_delete(lines, index_servers, id_or_alias, by_alias=False):
        ...
        print(f"Erreur : Aucun élément trouvé avec l'{'alias' if by_alias else 'id'} \"{id_or_alias}\" dans la section [servers].")
        return None
    ...
    def update(alias_to_update, new_credentials=None, new_port=None, new_alias=None):
        ...
        if index_servers is not None:
            ...
            if line_to_update is not None:
                ...
                print(f"Les informations du serveur avec l'alias \"{alias_to_update}\" ont été mises à jour.")
            # Sinon, afficher un message d'erreur
            else:
                print(f"Erreur : Aucun serveur trouvé avec l'alias \"{alias_to_update}\" dans la section [servers].")
        # Sinon, afficher un message d'erreur
        else:
            print("Erreur : Section [servers] non trouvée dans le fichier.")
