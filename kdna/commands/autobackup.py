...
def is_valid_cron(cron):
    if get_type_cron(cron) == ':':
        ...
        if len(cron_parts) != 5:
            return False
        if (0 >= cron_parts[0] <= 59 and 0 >= cron_parts[1] <= 23 and 1 >= cron_parts[2] <= 31 and 1
                >= cron_parts[
            3] <= 12):
            return True
        return False
...
def get_custom_cron(sched_part_type: str, cond_inf: int, cond_sup: int):
    ...
    :param sched_part_type: le type de la partie manquante de custom_cron à demander à l'utilisateur
     (e.g. 'Hour')\n
    ...
    while incorrect:
        schedule_part = input(
            f">Entrez une valeur numérique comprise entre {cond_inf} et {cond_sup} (inclus) pour '"
            f"{sched_part_type}', ou entrez 'help' pour afficher les correspondances : ")
        if schedule_part.lower() == "help":
            print(
                "Minute (0 - 59)\nHeure (0 - 23)\nJour du mois (1 - 31)\nMois (1 - 12)\nJour de la"
                " semaine (0 - 6) (dimanche est 0)\nNe rien entrer pour ne pas préciser.")
...
@click.option('-n', '--nameofcron', nargs=1, required=True, help="entrer le nom du"
                                                                 " cron")
...
@click.option('-d', '--date', nargs=1, required=True, help="entrer la date de la "
                                                           "première backup [ xxxx-xx-xx ]")
@click.option('-s', '--server', nargs=1, required=True, help="entrer l'id du serveur")
@click.option('-p', '--path', nargs=1, required=True, help="entrer le chemin de la"
                                                           " backup")
def schedule(idcron, nameofcron, tag, cron_schedule, custom_cron, date, server, path):
    ...
    Arguments obligatoires :\n
        ...
        \t- <custom_cron>: le schedule personnalisé de l'auto-backup, obligatoire si l'option
        custom a été séléctionnée\n
        \tSi l'argument n'a pas été saisi, le programme rentre en mode interactif et attend des
        entrées de l'utilisateur pour compléter custom_cron.
    """
    ...
    if cron_schedule == 'custom':
        if not custom_cron:  # custom_cron n'est pas donné en argument
            ...
            custom_cron = concatenate_custom_cron()  # le custom_cron est donc demandé en input
            # interactif
...
@autobackup.command()
@click.option('-i', '--idcron', nargs=1, required=True, help="entrer l'id du cron à"
                                                             " supprimer")
def delete(idcron):
    """
...
@click.option('-i', '--idcron', nargs=1, required=True, help="entrer l'id du cron à "
                                                             "mettre à jour")
@click.option('-t', '--tag', nargs=2, required=False, help="entrer le tag du cron à "
                                                           "mettre à jour et le tag mis à jour")
@click.argument('cron_schedule', type=click.Choice(['daily', 'monthly', 'weekly',
...
@click.option('-p', '--path', nargs=1, required=False, help="entrer le chemin de la"
                                                            " nouvelle backup")
def update(idcron, tag, cron_schedule, custom_cron, date, path):
    """
            Commande pour mettre à jour une backup régulière\n
            \t- <cron_schedule> : le schedule de l'auto-backup à mettre à jour ['daily', 'monthly',
             'weekly', 'custom']\n
            \t- <custom_cron> : le schedule personnalisé à mettre à jour de l'auto-backup,
            obligatoire si l'option custom a été séléctionnée\n
            \tSi l'argument n'a pas été saisi, le programme rentre en mode interactif et attend des
             entrées de l'utilisateur pour compléter custom_cron.
    """
