...
@click.option('-e', '--encrypt', default=True, required=False, help="Encrypt les ç"
...
@click.option('-r', '--new_repo', default='', required=False, help="entrer le répertoire"
                                                           " de sauvegarde Ex: /home/user/backup")
@click.option('-p', 'port', default='', required=False, help="entrer le nouveau port")
@click.option('-ad', 'new_address', default='', required=False, help="entrer la nouvelle"
                                                                     " adresse")
@click.option('-a', 'new_alias', default='', required=False, help="entrer le nouvel"
                                                                  " alias")
@click.option('-e', '--encrypt',default='', required=False, help="Encrypt les "
                                 "sauvegardes sur le serveur (valeur par defaut : True) Ex: False")
def update(alias, new_repo, port, new_address, encrypt, new_alias):
    ...
    """
    serverService = ServerService()
    if (alias and new_address or alias and new_repo or alias and port or alias and encrypt or alias
            and new_alias):
